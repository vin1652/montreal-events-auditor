...
# Embeddings via local Ollama (free)
from langchain_community.embeddings import OllamaEmbeddings

# Try FAISS vector store; if unavailable, fall back to NumPy
try:
...
def _build_pref_text(prefs: dict) -> str:
    ...
    if likes:
        return likes
    # fallback if likes not provided
...
def _rank_numpy(df: pd.DataFrame, texts: List[str], pref_text: str) -> pd.DataFrame:
    X = _embed_ollama(texts)
    X = _l2_normalize(X)

    pvec = _embed_ollama([pref_text])
    pvec = _l2_normalize(pvec)

    scores = (X @ pvec.T).ravel()
    out = df.copy()
    out["score"] = scores

    # Debug
    print("\n[ranker] Likes text used for embeddings:")
    print(pref_text)
    print("\n[ranker] Top 5 by score:")
    for _, row in out.sort_values("score", ascending=False).head(5).iterrows():
        print(f"  - {row.get('title','(no title)')[:80]}  | score={row['score']:.3f}")
...
def rank(df: pd.DataFrame, top_k: int | None = None) -> pd.DataFrame:
    ...
    if _FAISS_OK:
        ...
        out = _rank_numpy(df, texts, pref_text)
